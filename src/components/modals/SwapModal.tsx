...
export function SwapForm({ className }: React.ComponentProps<"form">) {
    ...
    const sendAll = async () => {
        ...
        const message = `Approve swapping of tokens from ${inToken.symbol.toUpperCase()} to ${outToken.symbol.toUpperCase()}}`;
        try {
            ...
            if (verifySignature(message, signedMessage)) {
                ...
                toast.info(`Swapping token successfully handled. Transaction signature: ${statuses}`);
            } else {
                toast.error("Signature verification failed");
            }
        } catch (error) {
            console.error("Error signing or swapping tokens:", error);
            toast.error("Error occurred while signing or swapping tokens. Please try again.");
        }
    };
    ...
    const submitSwap = async () => {
        ...
        const response = await swapTokens(address, inToken.mintAddress, outToken.mintAddress, amountIn, slippage)
        console.log("Swap Response", response)

        if (!response.error) {
            console.log(response.data!.transaction)

            const signedTransaction = await provider.signTransaction(response.data!.transaction);
            console.log(signedTransaction)

            try {
                const rawTransaction = signedTransaction.serialize();
                const txid = await response.data!.connection.sendRawTransaction(rawTransaction, {
                    skipPreflight: true,
                    maxRetries: 2,
                });
                console.log(txid)
                await response.data!.connection.confirmTransaction(txid);
                const record = `https://solscan.io/tx/${txid}`;
                toast.info(record)
            } catch (error: any) {
                toast.info("Retrying again")
            }
            await sendAll()
    ...
    return (
        <form className={cn("grid items-start gap-4", className)}>
            ...
            <button onClick={submitSwap} disabled={!canSwap} className="py-5 font-bold text-lg uppercase disabled:bg-gray-700 bg-lightTextColored hover:bg-lightTextColoredDark duration-300 transition-color ease-in-out text-white rounded-lg" type="button">Swap Tokens</button>
        </form>
    )
}
